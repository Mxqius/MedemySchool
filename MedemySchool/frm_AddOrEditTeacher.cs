...
{
    public partial class frm_AddOrEditTeacher : Form
    {
        ...
        bool EditTeacher()
        {
            try
            {
                SqlCommands.EditTeachers(ID, txt_username.Text, txt_password.Text, txt_firstname.Text, txt_lastname.Text, txt_phonenumber.Text,
                          txt_email.Text, txt_address.Text, txt_nationalcode.Text, GenderCheck(), StatusCheck(), ims.ConvertImageToByte(pic_teacher.Image), dp_date_birth.Value, TimeAndDate.GetDate(), TimeAndDate.GetTime());
                return true;
            }
            catch (Exception)
            {
        ...
        bool AddTeacher()
        {
            try
            {
                SqlCommands.AddTeachers(txt_username.Text, txt_password.Text, txt_firstname.Text, txt_lastname.Text, txt_phonenumber.Text,
                          txt_email.Text, txt_address.Text, txt_nationalcode.Text, GenderCheck(), StatusCheck(), ims.ConvertImageToByte(pic_teacher.Image), dp_date_birth.Value, TimeAndDate.GetDate(), TimeAndDate.GetTime());
                return true;
            }
            catch
            ...
            if (!check)
            {
                if (!EditMode)
                {
                    ...
                    if (res)
                    {
                        ...
                        lbl_log.Text = "New teacher added successfully";
                    }
                    else
                    {
                        lbl_log.ForeColor = Color.Red;
                        lbl_log.Text = "There was a problem adding a new teacher, please try again";
                    }
                }
                else
                {
                    bool res = EditTeacher();
                    if (res)
                    {
                        lbl_log.ForeColor = Color.Green;
                        lbl_log.Text = "Teacher information changed successfully";
                    }
                    else
                    {
                        lbl_log.ForeColor = Color.Red;
                        lbl_log.Text = "There was a problem changing the teacher information. Please try again";
                    }
                }
            }
        }
        ...
        private void btn_upload_pic_Click(object sender, EventArgs e)
        {
            ...
            {
                pic_teacher.Image = Image.FromFile(ofd_picture.FileName);
            }
        }

        private void frm_AddOrEditTeacher_Load(object sender, EventArgs e)
        {
            if (ID == 0)
            {
                lbl_title.Text = "Add Teacher";
            }
            else
            {
                lbl_title.Text = "Edit Teacher";
                DataTable dt = SqlCommands.TeacherSelectRow(ID);
                ...
                if (dt.Rows[0][10] == System.DBNull.Value)
                    pic_teacher.Image = Properties.Resources.defult_profile;
                else
                    pic_teacher.Image = ims.ConvertByteToImage((byte[])dt.Rows[0][10], Properties.Resources.defult_profile);
                SetGender();
                SetStatus();
            }
